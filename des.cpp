...
vector<string> generateRoundKeys(string key64) {
    ...
    for(int r = 0; r < 16; r++) {
        ...
        string rk48 = "";
        for(int i = 0; i < 48; i++) rk48 += combined[PC2[i]-1];
        roundKeys.push_back(rk48);
    }
    return roundKeys;
}

// ================= FEISTEL FUNCTION =================
string sbox(string in48) {
    string out32 = "";
    for(int i = 0; i < 8; i++) {
        string block = in48.substr(i*6, 6);
...
// ================= DES CORE =================
string desEncrypt(string pt64, vector<string>& roundKeys) {
    ...
    for(int i = 0; i < 16; i++) {
        string temp = R;
        R = xor_str(L, feistel(R, roundKeys[i]));
        L = temp;
    }
    return permute(R + L, FP, 64);
}

string desDecrypt(string ct64, vector<string>& roundKeys) {
    reverse(roundKeys.begin(), roundKeys.end());  // Quan trọng!
    return desEncrypt(ct64, roundKeys);
}

// ================= TripleDES =================
string tripleDESEncrypt(string pt64, string k1, string k2, string k3) {
    vector<string> rk1 = generateRoundKeys(k1);
    vector<string> rk2 = generateRoundKeys(k2);
    vector<string> rk3 = generateRoundKeys(k3);

    string temp = desEncrypt(pt64, rk1);
    temp = desDecrypt(temp, rk2);
    temp = desEncrypt(temp, rk3);
    return temp;
}
...
string tripleDESDecrypt(string ct64, string k1, string k2, string k3) {
    ...
    string temp = desDecrypt(ct64, rk3);
    temp = desEncrypt(temp, rk2);
    temp = desDecrypt(temp, rk1);
    return temp;
}
...
int main() {
    ...
    if(mode == 1) { // DES Encrypt
        ...
        for(size_t i = 0; i < pt.length(); i += 64) {
            string block = pt.substr(i, 64);
            result += desEncrypt(block, roundKeys);
        }
        cout << result << endl;
    }
    else if(mode == 2) { // DES Decrypt
        string ct, key;
        getline(cin, ct); getline(cin, ct); ct = cleanBinary(ct);
        getline(cin, key); key = cleanBinary(key);

        vector<string> roundKeys = generateRoundKeys(key);
        string result = desDecrypt(ct, roundKeys);  // Note: function handles reverse
        cout << result << endl;
    }
    else if(mode == 3) { // TripleDES Encrypt
        string pt, k1, k2, k3;
        getline(cin, pt); getline(cin, pt); pt = cleanBinary(pt);
        getline(cin, k1); k1 = cleanBinary(k1);
        getline(cin, k2); k2 = cleanBinary(k2);
        getline(cin, k3); k3 = cleanBinary(k3);

        if(pt.length() != 64) while(pt.length() % 64 != 0) pt += '0';
        string result = tripleDESEncrypt(pt.substr(0,64), k1, k2, k3);
        cout << result << endl;
    }
        ...
        string result = tripleDESDecrypt(ct, k1, k2, k3);
        cout << result << endl;
    }
